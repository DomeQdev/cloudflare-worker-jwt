...
class JWT {
    ...
    async sign(payload, secret, options = { algorithm: 'HS256' }) {
        ...
        const partialToken = `${Base64URL.stringify(_utf8ToUint8Array(JSON.stringify({ alg: options.algorithm, kid: options.keyid })))}.${Base64URL.stringify(_utf8ToUint8Array(payloadAsJSON))}`
        let keyFormat = 'raw'
        let keyData
        if (secret.startsWith('-----BEGIN')) {
            keyFormat = 'pkcs8'
            keyData = _str2ab(atob(secret.replace(/-----BEGIN.*?-----/g, '').replace(/-----END.*?-----/g, '').replace(/\s/g, '')))
        } else
            keyData = _utf8ToUint8Array(secret)
        const key = await crypto.subtle.importKey(keyFormat, keyData, importAlgorithm, false, ['sign'])
        const signature = await crypto.subtle.sign(importAlgorithm, key, _utf8ToUint8Array(partialToken))
    ...
    async verify(token, secret, options = { algorithm: 'HS256' }) {
        ...
        if (secret.startsWith('-----BEGIN')) {
            ...
            keyData = _utf8ToUint8Array(secret)
        const key = await crypto.subtle.importKey(keyFormat, keyData, importAlgorithm, false, ['sign'])
        const res = await crypto.subtle.sign(importAlgorithm, key, _utf8ToUint8Array(tokenParts.slice(0, 2).join('.')))
